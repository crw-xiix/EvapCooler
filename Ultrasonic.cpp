...
unsigned int Ultrasonic::timing() {
  ...
  previousMicros = micros();
//  while (digitalRead(echo) && (micros() - previousMicros) <= timeout) val++; // wait for the echo pin LOW or timeout
  while (digitalRead(echo) && (val<32767)) val++; // wait for the echo pin LOW or timeout
 ...
 */
float Ultrasonic::read(uint8_t und) {
	
  return timing() / ((float) und) / 2.0f;  //distance by divisor
}
 ...
 * Use read() instead.
 */
float Ultrasonic::distanceRead(uint8_t und) {
  return read(und);
}
